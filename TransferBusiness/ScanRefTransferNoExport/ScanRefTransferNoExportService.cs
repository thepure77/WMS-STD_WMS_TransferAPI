...
{
    public class ScanRefTransferNoExportService
    {
        #region TransferDbContext
        private TransferDbContext db;

        public ScanRefTransferNoExportService()
        {
            db = new TransferDbContext();
        }

        public ScanRefTransferNoExportService(TransferDbContext db)
        {
            this.db = db;
        }
        #endregion

        #region FilterScanTransferExport
        public List<listTaskViewModel> FilterScanTransferExport(ScanRefTransferNoExportViewModel model)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.goodsTransfer_No) && (!string.IsNullOrEmpty(model.username)))
                {
                    items = (from gt in db.IM_GoodsTransfer
                             ...
                             where gt.GoodsTransfer_No == model.goodsTransfer_No && gt.Document_Status == 2 && tgt.UserAssign == model.username
                    ...
                    items = (from gt in db.IM_GoodsTransfer
                             join tgti in db.im_TaskTransferItem on gt.GoodsTransfer_Index equals tgti.Ref_Document_Index
                             join tgt in db.im_TaskTransfer on tgti.TaskTransfer_Index equals tgt.TaskTransfer_Index
                             where gt.Document_Status == 2 && tgt.UserAssign == model.username
                             group gt by new { gt.GoodsTransfer_Index, gt.GoodsTransfer_No, tgt.TaskTransfer_No, tgt.TaskTransfer_Index, gt.GoodsTransfer_Date } into g
                             select new listTaskViewModel
                             {
                                 goodsTransfer_Index = g.Key.GoodsTransfer_Index,
        ...
        public List<listTaskViewModel> FilterGoodsTransferExport(ScanRefTransferNoExportViewModel model)
        {
            try
            {
        ...
        public GoodsTransferViewModel ScanTransfer(string transferNo, string user)
        {
            try
            {
                ...
                var queryResult = (from gt in db.IM_GoodsTransfer
                                   join tgti in db.im_TaskTransferItem on gt.GoodsTransfer_Index equals tgti.Ref_Document_Index
                                   join tgt in db.im_TaskTransfer on tgti.TaskTransfer_Index equals tgt.TaskTransfer_Index
                                   where gt.GoodsTransfer_No == transferNo && gt.Document_Status == 2
        ...
        #region Confirm
        public string Confirm(ScanRefTransferNoExportViewModel model)
        {
        ...
        public string ConfirmTaskTransfer(ScanRefTransferNoExportViewModel model)
        {
            String State = "Start " + model.sJson();
            String msglog = "";
            var olog = new logtxt();
                ...
                else
                {
                    ...
                    if ((GTI.Location_Id != GTI.Location_Id_To) || (GTI.ItemStatus_Id != GTI.ItemStatus_Id_To) || (GTI.Tag_No != GTI.Tag_No_To) || GTI.ERP_Location != GTI.ERP_Location_To)
                    {
                        ...
                        else
                        {
                            ...
                            olog.logging("ScanRefTransferNo", "Befor InsertBinCardTransferNotCreateTagItem");
                            var CreateTagHeader = utils.SendDataApi<string>(new AppSettingConfig().GetUrl("InsertBinCardTransferNotCreateTagItem"), binCardModel.sJson());
                            olog.logging("ScanRefTransferNo", CreateTagHeader.ToString());
                            if (CreateTagHeader != null)
                            {
                                olog.logging("ScanRefTransferNo", "in CreateTagHeader");
                            ...
                            olog.logging("ScanRefTransferNo", "After InsertBinCardTransferNotCreateTagItem");
                        }
                    }
                    #endregion
            ...
            try
            {
                ...
                if (goodsTransfer != null)
                {
                    ...
                    foreach (var item in goodsTransferItem)
                    {
                        ...
                        ScanRefTransferNoExportViewModel scanRef = new ScanRefTransferNoExportViewModel();
        ...
        public string UpdateRePutaway(ScanRefTransferNoExportViewModel model)
        {
            String State = "Start " + model.sJson();
            String msglog = "";
            var olog = new logtxt();
