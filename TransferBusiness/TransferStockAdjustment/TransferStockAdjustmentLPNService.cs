...
{
    public class TransferStockAdjustmentLPNService
    {
        ...
        public List<BinBalanceDocViewModel> ScanLPN(BinBalanceDocViewModel data)
        {
            try
            {
                using (var context = new TransferDbContext())
                {
                    ...
                    //var queryResult = context.wm_BinBalance.FromSql("sp_GetBinBalance").Where(c => c.Tag_No == data.Tag_No && c.Owner_Index == data.Owner_Index && c.BinBalance_QtyBal != 0 && c.BinBalance_QtyBal - c.BinBalance_QtyReserve > 0).OrderBy(o => o.GoodsReceive_EXP_Date).ToList();
        ...
        //            {
        //                data.StockAdjustmentIndex = Guid.NewGuid();
        //            }
        ...
        //                {
        //                    GI.GoodsIssueIndex = Guid.NewGuid();
        //                }
        ...
        //                GILocationResult.Create_By = data.CreateBy;
        ...
        //                        var queryGR = context2.IM_GoodsReceives.FromSql("sp_GetGoodsReceiveStock @strwhere", strwhereGR).FirstOrDefault();

        //                        GRIndex = queryGR.GoodsReceive_Index.ToString();
        //                    }
        ...
        //                    BinCard.GoodsReceive_Index = new Guid(GRIndex);
        ...
        //                    //var UnitWeight = queryResult.BinBalance_Weigh
        ...
        //                    //                      "  BinBalance_WeightBal  =  " + queryResult.BinBalance_WeightBal + "" +
        //                    //                      "  ,BinBalance_VolumeBal  = " + queryResult.BinBalance_VolumeBal + "" +
        //                    //                      " where Convert(Varchar(200),BinBalance_Index) ='" + queryResult.BinBalance_Index + "'";
...

        {
            try
            {
                using (var context = new TransferDbContext())
                {
                    ...
                    //&& c.Tag_No == data.Tag_No
                    //&& c.Owner_Index == data.OwnerIndex
        ...
        public List<SumQtyBinbalanceViewModel> SumQty(SumQtyBinbalanceViewModel data)
        {
            try
            {
                using (var context = new TransferDbContext())
                {
                    ...
                    {
                        //var queryResult = context.View_SumQtyBinbalanceLPN.FromSql("sp_GetViewSumQtyBinbalanceLPN").Where(c => c.Tag_No == data.Tag_No && c.Warehouse_Index == data.WareHouseIndex && c.Owner_Index == data.ownerIndex).ToList();
                        ...
                        var queryResult = context.View_SumQtyBinbalanceLPN.FromSql("sp_GetViewSumQtyBinbalanceLPN @strwhere", strwhere).ToList();
        ...
        public String CheckScanLPN(BinBalanceDocViewModel data)
        {
            try
            {
                using (var context = new TransferDbContext())
                {
                    ...
                    string SqlCheckTag = " and Convert(Nvarchar(50),Tag_No) = N'" + data.Tag_No + "' ";
                    var strwhereCheckTag = new SqlParameter("@strwhere", SqlCheckTag);
                    var CheckTag = context.wm_Tag.FromSql("sp_GetTag @strwhere", strwhereCheckTag).FirstOrDefault();

                    SqlWhere = " and Tag_No = N'" + data.Tag_No + "'" +
                               " and Location_Index in (select Location_Index from ms_Location where  LocationType_Index = '76C758AA-C216-406C-BDF6-14018C87CAE1')";
                    var strwhere = new SqlParameter("@strwhere", SqlWhere);
                    var CheckLocation = context.wm_BinBalance.FromSql("sp_GetBinBalance @strwhere", strwhere).FirstOrDefault();


                    if (CheckTag != null && CheckLocation != null)
                    {
                        return "true";
                    }
