...
{
    public class GoodsTransferItemService
    {
        private TransferDbContext db;
        private MasterConnectionDbContext dbMaster;

        public GoodsTransferItemService()
        {
            db = new TransferDbContext();
            dbMaster = new MasterConnectionDbContext();
        }

        public GoodsTransferItemService(TransferDbContext db , MasterConnectionDbContext dbMaster)
        {
            this.db = db;
            this.dbMaster = dbMaster;
        }

        public List<GoodsTransferItemViewModel> GoodsTransferItem(string id,bool ischkQI)
        {
            try
            {
                ...
                if (ischkQI)
                {
                    result = result.GroupBy(g => new { g.udf_1, g.udf_2, g.udf_3,g.documentRef_No1 }).Select(s => new GoodsTransferItemViewModel { udf_1 = s.Key.udf_1, udf_2 = s.Key.udf_2, udf_3 = s.Key.udf_3,documentRef_No1=s.Key.documentRef_No1 }).ToList();
                }
        ...
        public string AddNewLocation(GoodsTransferItemViewModel data)
        {
            String State = "Start";
            String msglog = "";
            var olog = new logtxt();
            ...
            if (data.goodsReceive_MFG_Date.toDate() != data.goodsReceive_MFG_Date_To.toDate())
            {
                GoodTransferItem.GoodsReceive_MFG_Date_To = data.goodsReceive_MFG_Date_To.toDate();
                GoodTransferItem.Update_By = data.update_By;
                GoodTransferItem.Update_Date = DateTime.Now;
            }

            if (data.goodsReceive_EXP_Date.toDate() != data.goodsReceive_EXP_Date_To.toDate())
            {
                GoodTransferItem.GoodsReceive_EXP_Date_To = data.goodsReceive_EXP_Date_To.toDate();
                GoodTransferItem.Update_By = data.update_By;
                GoodTransferItem.Update_Date = DateTime.Now;
            }
