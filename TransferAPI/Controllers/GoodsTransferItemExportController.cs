...
{
    [Route("api/TransferItemExport")]
    public class GoodsTransferItemExportController : Controller
    {
        ...
        [HttpPost("CheckLocationOnGround")]
        public IActionResult CheckLocation([FromBody]JObject body)
        {
            try
            {
                var service = new GoodsTransferItemExportService();
                var Models = new GoodsTransferItemExportViewModel();
                Models = JsonConvert.DeserializeObject<GoodsTransferItemExportViewModel>(body.ToString());
